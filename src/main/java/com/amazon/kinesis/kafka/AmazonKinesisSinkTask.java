			...
			if (singleKinesisProducerPerPartition) {
				producerMap.values().forEach(producer -> {
					...
					while (producer.getOutstandingRecordsCount() > outstandingRecordsThreshold) {
						try {
							// Pausing further
							sinkTaskContext.pause((TopicPartition[]) sinkTaskContext.assignment().toArray(new TopicPartition[0]));
							pause = true;
					...
					if (pause)
						sinkTaskContext.resume((TopicPartition[]) sinkTaskContext.assignment().toArray(new TopicPartition[0]));
				});
				return true;
			} else {
				int sleepCount = 0;
